...
test("portNumbers throws on invalid ranges", (t) => {
	t.throws(
		() => portNumbers("abc", 3000),
		{ instanceOf: TypeError },
		"`from` is not an integer number"
	);
	t.throws(
		() => portNumbers(3000, "abc"),
		{ instanceOf: TypeError },
		"`to` is not an integer number"
	);

	t.throws(
		() => portNumbers(1023, 1024),
		{ instanceOf: RangeError },
		"`from` is less than the minimum port"
	);
	t.throws(
		() => portNumbers(65_536, 65_536),
		{ instanceOf: RangeError },
		"`from` is greater than the maximum port"
	);

	t.throws(
		() => portNumbers(1024, 1023),
		{ instanceOf: RangeError },
		"`to` is less than the minimum port"
	);
	t.throws(
		() => portNumbers(65_535, 65_537),
		{ instanceOf: RangeError },
		"`to` is greater than the maximum port"
	);
